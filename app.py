...
# Coordinates are derived automatically from the CONFIG bbox above.
# To forecast a different time window, adjust weather_start / weather_end.
# Two forecast models are used automatically:
#   harmonie -> high resolution, up to 3 days ahead
#   ecmwf    -> medium range,    up to 9 days ahead

FMI_URL = "https://opendata.fmi.fi/wfs/eng"

now = datetime.now(timezone.utc)

weather_start = now                     # start of forecast window
weather_end   = now + timedelta(days=2) # end   of forecast window
...
# Collect records: { time_string -> { parameter -> value } }
weather_records = {}
...
time_keys = list(weather_records.keys())
...
for time_str, vals in weather_records.items():
    ...
    # Skip timesteps without at least a temperature reading
    if temp is None:
        continue
    ...
    # Spread markers in a grid so they don't stack on a single point.
    # FMI returns the same lat/lon for every timestep; the grid makes
    # the full forecast timeline browsable on the map.
    dt_index   = time_keys.index(time_str)
